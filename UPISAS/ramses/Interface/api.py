...
def fetch_system_architecture():
    url = "http://localhost:32778/rest/systemArchitecture"
...
def fetch_instance_configuration(serviceName, implementationId):
    url = "http://localhost:32778/rest/service/{}/configuration?implementationId={}".format(serviceName,
                                                                                            implementationId)
    try:
        response = requests.get(url)
        response.raise_for_status()
...
def fetch_service_snapshot(serviceName):
    url = "http://localhost:32778/rest/service/{}/snapshot".format(serviceName)
    try:
        response = requests.get(url)
        response.raise_for_status()
